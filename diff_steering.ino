...
#define CMD_MAX_LENGTH 8     //Set the max command length for this program
  ...
  if (Xbox.XboxReceiverConnected) {
    if (Xbox.Xbox360Connected[CONTROLLER]) {
      ...
      if (Xbox.getAnalogHat(RightHatY) > DEAD_ZONE || Xbox.getAnalogHat(RightHatY) < -DEAD_ZONE) {
        ...
        nJoyX = map(Xbox.getAnalogHat(RightHatX), -32767, 32767, -127, 127);
        nJoyY = map(Xbox.getAnalogHat(RightHatY), -32767, 32767, -127, 127);
        if (nJoyY >= 0) {
          // Forward
        ...
        RFoot.writeMicroseconds(map(nMotMixR, -127, 127, 1750, 1250));
        LFoot.writeMicroseconds(map(nMotMixL, -127, 127, 1750, 1250));
      }
    }
    //if the stick is in the DEAD_ZONE - shut down the motor
    if ((Xbox.getAnalogHat(RightHatY) < DEAD_ZONE && Xbox.getAnalogHat(RightHatY) > -DEAD_ZONE) && (Xbox.getAnalogHat(RightHatX) < DEAD_ZONE && Xbox.getAnalogHat(RightHatX) > -DEAD_ZONE)) {
      RFoot.writeMicroseconds(1500);
      LFoot.writeMicroseconds(1500);
    }
